...
DBeaver-Plugin fuer AI-gestuetzten Chat mit Tabellenkontext.

## Features

- Eigene Chat-View: `ch.so.agi.dbeaver.ai.views.chat`
- Send/Stop/Clear Context direkt in der Chat-View
- `#`-Autocomplete im Chat-Eingabefeld fuer Referenzen im Format `datasource.schema.table`
- `@sql` fuer die aktive oder markierte SQL-Query aus dem SQL-Editor
- Mention-Parsing vor dem Senden
- Kontextanreicherung je referenzierter Tabelle:
  - Datenbanktyp (primaer Treibername, fallback SQL-Dialect)
  - DDL (primaer via DBeaver-DDL-Generator ohne Header, sonst robuster Fallback)
  - Sample-SQL (`SELECT * ... LIMIT n`)
- Sichtbare Warnung bei Token-Budget-Kuerzung des Tabellenkontexts
- LLM-Streaming ueber LangChain4j (`langchain4j` + `langchain4j-open-ai`)
- Konfigurierbarer System-Prompt
- Prompt-Dateien oeffnen/speichern (`Open...`, `Save`, `Save As...`)
- Lokales Log aller gesendeten User-Prompts in `~/.dbeaver-ai-messages.log`
- Settings in regulaeren DBeaver-Preferences
- API-Token in DBeaver Secret Storage (nicht im Klartext in Preferences)

## Installation in DBeaver

1. In DBeaver `Help -> Install New Software...` oeffnen.
2. Als Update Site `https://dbeaver.sogeo.services/updates` eintragen.
3. Warten, bis die Site geladen ist, und das AI-Feature auswaehlen.
4. Den Installationsdialog durchklicken. Je nach DBeaver-/Netzwerkzustand muss dabei haeufig mehrmals bestaetigt oder erneut auf `Next` bzw. `Finish` geklickt werden, bis alle Artefakte geladen sind.
...
| Max Columns per Sample | `ch.so.agi.dbeaver.ai.maxColumnsPerSample` | `30` | Aus Kompatibilitaetsgruenden sichtbar, derzeit aber in der UI deaktiviert und ohne Wirkung, solange Sample Rows global abgeschaltet sind. Werte `<1` werden intern auf `1` angehoben. |
...
| Max Context Tokens | `ch.so.agi.dbeaver.ai.maxContextTokens` | `4000` | Token-Budget fuer den Tabellenkontext. Bei Ueberschreitung wird Kontext gekuerzt und im Chat sichtbar gewarnt. Werte `<100` werden auf `100` angehoben. |
| Autocomplete Candidate Scan Limit | `ch.so.agi.dbeaver.ai.mentionCandidateLimit` | `100000` | Max. Anzahl Tabellen, die beim Metadaten-Scan in den internen `#`-Autocomplete-Katalog geladen werden. Hoehere Werte erhoehen Vollstaendigkeit, koennen den Initial-Scan aber verlangsamen. Werte `<1` werden auf `1` angehoben. |
...
5. Antwort wird streaming-basiert im Chat angezeigt; bei gekuerztem Tabellenkontext erscheint eine sichtbare Warnung.
6. `Clear Context` leert den sichtbaren Transcript und die Chat-History, ohne den aktuellen Prompt im Eingabefeld zu loeschen.
...
- Verlauf ist aktuell view-lokal (nicht ueber DBeaver-Neustart persistent).
- Bei unaufloesbaren Mentions wird eine Warnung ausgegeben, der Chat laeuft weiter.
- Sample Rows werden derzeit nicht an das Modell gesendet; die zugehoerigen Settings sind nur noch als deaktivierte Kompatibilitaetsfelder sichtbar.
